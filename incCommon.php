	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = array(
			...
			'Details' => array('oz4host Domains', '', 'table.gif', 'None'),
			'Calculation' => array('Calculation', '', 'table.gif', 'None')
		);
	...
	function get_sql_fields($table_name) {
		$sql_fields = array(
			'Details' => "`Details`.`ID` as 'ID', `Details`.`Due_Date` as 'Due_Date', `Details`.`Domain` as 'Domain', `Details`.`Sub_Domains` as 'Sub_Domains', `Details`.`Crazy_Domains` as 'Crazy_Domains', `Details`.`DNS_Provider` as 'DNS_Provider', CONCAT('$', FORMAT(`Details`.`Yearly_Fee`, 2)) as 'Yearly_Fee', `Details`.`Company_Name` as 'Company_Name', `Details`.`Contact` as 'Contact', `Details`.`Email_Address` as 'Email_Address', `Details`.`Alternative_Email` as 'Alternative_Email', `Details`.`Notes` as 'Notes'",
			'Calculation' => "`Calculation`.`ID2` as 'ID2', `Calculation`.`Total` as 'Total'",
		);
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> oz4host</a>
			</div>
